                                ...
                                <table class="display table table-striped table-hover datatabled">
                                    ...
                                    @foreach ($slider as $item)
                                        <tr>
                                            ...
                                            <td width="10">{{ $item->urutan }}</td>
                                            <td width="50"><img src="{{ ($item->thumbnail == '') ? asset('img/default.png') : url(Storage::url($item->thumbnail)) }}" alt="" width="120" class="p-2"></td>
                                            <td>
                                                {{ $item->title }}
                                            </td>
                                            <td>
                                                {{ Str::limit($item->subtitle, 30, '...') }}
                                            </td>
                                            <td>
                                                <a href="{{ $item->btn_link }}" class="btn btn-primary">{{ $item->btn_text }}</a>
                                            </td>
                                            <td width="50">
                                                <div class="d-flex justify-content-center">
                                                    <a href="{{ route('dashboard.slider.edit', $item->id) }}" class="btn btn-sm btn-warning mr-2">Edit</a>
                                                    <form action="{{ route('dashboard.slider.destroy', $item->id) }}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
